...
def process_all_batch_images():
    """OPTIMIZED batch processing - no UI updates during loop"""
    ...
    # SINGLE progress indication
    with st.spinner(f"Processing {len(st.session_state.quick_batch_images)} images..."):
        
        # LOCAL results (not session state during processing)
        local_results = {}
        images = st.session_state.quick_batch_images.copy()
        
        # NO STREAMLIT CALLS INSIDE THIS LOOP!
        for image_path in images:
            try:
                # Direct processing - no UI updates
    ...
    # SINGLE session state update at end
    st.session_state.quick_batch_results = local_results
    
    total_detections = sum(len(r['detections']) for r in local_results.values())
    st.success(f"Batch processing complete! {len(local_results)} images, {total_detections} total detections")
...
def handle_image_upload(uploaded_file):
    ...
    try:
        ...
        temp_path, storage_info = handle_single_image_upload(uploaded_file, "quick_detection")
